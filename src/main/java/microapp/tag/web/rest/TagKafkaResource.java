...
import microapp.tag.config.KafkaSseConsumer;
import microapp.tag.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/tag-kafka")
public class TagKafkaResource {

    private final Logger log = LoggerFactory.getLogger(TagKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public TagKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
